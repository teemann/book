...
int main(int argc, char* argv[]){
    ...
    if((server = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET){
        printf("Oops, something went wrong when creating the socket %d\n", WSAGetLastError());
		WSACleanup();
        return 1;
    }
    ...
    int ena = 1;
    setsockopt(server, SOL_SOCKET, SO_REUSEADDR, (const char*)&ena, sizeof(int));

    if(bind(server, (struct sockaddr*)&sinServer, sizeof(sinServer)) == SOCKET_ERROR){
        close(server);
		WSACleanup();
    ...
    while(1){
        ...
        if((client = accept(server, (struct sockaddr*)&sinClient, &slen)) == SOCKET_ERROR){
            closesocket(server);
			WSACleanup();
